...
export default function AdminPanel() {
  ...
  const addProject = (project) => {
    ...
    // Add API call to save project to the backend
  };

  const addClient = (client) => {
    setClients([...clients, client]);
    // Add API call to save client to the backend
  };
      ...
      <section className="mb-12">
        ...
        <div className="bg-white shadow-md rounded-md p-4 mb-4">
          <form onSubmit={(e) => {
            ...
            <textarea name="description" placeholder="Project Description" className="border p-2 mb-2 w-full"></textarea>
            <button type="submit" className="bg-blue-500 text-white px-4 py-2 rounded">Add Project</button>
          </form>
        </div>
        <ul>
          {projects.map((project, index) => (
            <li key={index} className="p-2 border mb-2">{project.name}</li>
          ))}
        </ul>
      </section>
      ...
      <section className="mb-12">
        ...
        <div className="bg-white shadow-md rounded-md p-4 mb-4">
          <form onSubmit={(e) => {
            ...
            <textarea name="description" placeholder="Client Description" className="border p-2 mb-2 w-full"></textarea>
            <input type="text" name="designation" placeholder="Client Designation" className="border p-2 mb-2 w-full" />
            <button type="submit" className="bg-blue-500 text-white px-4 py-2 rounded">Add Client</button>
          </form>
        </div>
        <ul>
          {clients.map((client, index) => (
            <li key={index} className="p-2 border mb-2">{client.name}</li>
          ))}
        </ul>
      </section>
      ...
      <section>
        ...
        <ul className="bg-white shadow-md rounded-md p-4">
          {subscribers.map((subscriber, index) => (
            <li key={index} className="border-b py-2">{subscriber.email}</li>
          ))}
        </ul>
      </section>
    </div>
